...
#!/bin/bash
result=$(curl -s --user SimpleAuthClient:secret -X POST "https://oauth.magticom.ge/auth/oauth/token?grant_type=authorization_code&redirect_uri=http://localhost:8082/pitalo/login&client_id=SimpleAuthClient&code=g6IOOS"   )
echo "$result"
